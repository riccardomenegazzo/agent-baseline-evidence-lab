...
Before a before/after result is treated as causal evidence, the lab checks that the material measured invariants are the same and that a governance treatment actually changed.

The protocol is fail-closed. Missing evidence does not become a pass.
...
The first version of the protocol requires evidence for all of these dimensions:
...
| Task ID | agent-run capsule/config | Keeps the declared task stable. |
| Task SHA-256 | agent-run capsule | Proves the actual prompt/task bytes are the same without persisting the prompt. |
| Initial workspace SHA-256 | agent-run capsule | Proves both runs start from the same code/content state. |
| Agent runtime | agent-run capsule | Prevents changing the agent implementation between arms. |
| Docker Sandbox runtime fingerprint | `CON-03` runtime evidence | Prevents an observed Docker runtime-version change from being silently treated as governance effect. |
...
A controlled before/after experiment requires this treatment fingerprint to differ.

If all invariants match but the treatment is unchanged, the pair is `NOT_ELIGIBLE`: it is a repeatability/control observation, not a treatment experiment.
...
One or more required invariants could not be measured from the supplied evidence.
...
A strong PoC should be able to answer two different questions without conflating them:

1. **Did the control/evidence posture change?** — Governance Delta.
2. **Were the two observations sufficiently controlled to discuss the governance change as a causal factor?** — Controlled Experiment Protocol.

That separation prevents an attractive before/after dashboard from becoming an unsupported security claim.
